...
while 1: 
    ...
    if sc and bufpos > len(buf):
        ...
        except KeyboardInterrupt: print "^C" ; buf = [] ; agg = "" ; p = ""
        except: p = [""]
    ...
    if len(token):
        ##print "prevtoken: ",prevtoken,"\"" + agg + "\"","pfound:",pfound
        if token in ["quit", "exit"] and bufpos == 1: alexq(alexhist)
        elif token in ["help", "pserver", "isoname", "minusname", "cat", "arrdo"] and (bufpos == 1 or prevtoken == "|"): token = pyprefix + alexver + ".py --" + token ; x = 1
    ...
    if token != ";" and bufpos > 1: agg += " " + token ; x = 0
    if token == ";" and bufpos > 1: 
        alexr = alexshell(agg) ; agg = "" ; prevtoken = ""
