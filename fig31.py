...
,("highlight colorcode", "output (shared-line) change background color of upcoming text tocolorcode 0-15")
,("locate row column", "output (shared-line) move to textmode position at row, column")
,("pset x y c", "output (shared-line) draw dot at location (x, y) in colorcode c (0 - 15)")
,("line x1 y1 x2 y2 c", "output (shared-line) draw line from (x1, y1) to (x2, y2) in colorcode c (0-15)")
...
,("fig/next/nextin/wend", "fig (interchangeable) function --\\own\\line finalize a block (started by if/while/function/for/forin")
...
def outfilewrite(outb, p):
    ...
    #global vrck
    #vrck += p.strip()
    #if inle: print color(5, 0) + p.rstrip() ; p=input() ; quit()
...
def figfsp(p):
    ...
    for fp in enumerate(fsp):
        if flg == 0 and fp[1] in list(cmds.keys()):
...
addtoout[0] = """import sys, codecs, os, io
from sys import stdout
stdin=io.open(sys.stdin.fileno(),'r',encoding='latin-1')
...
def figpset(z, x, y, c):
    ...
    if figraphics == 0:
        if x > -1 and y > -1:
            figcolortext(c, c)
            figlocate(0, int(y) + 1, int(x) + 1) ; stdout.write(chr(9608))
            sys.stdout.flush()
    if figraphics == 1:
        if x > -1 and y > -1:
...
def figline(z, x, y, x2, y2, c):
    ...
    if figraphics == 0:
        if x > -1 and y > -1 and x2 > -1 and y2 > -1:
            ...
            figlocate(0, int(y) + 1, int(x) + 1) ; stdout.write(chr(9608))
            figlocate(0, int(y2) + 1, int(x2) + 1) ; stdout.write(chr(9608))
            sys.stdout.flush()
    if figraphics == 1:
        if x > -1 and y > -1 and x2 > -1 and y2 > -1:
            yourscreen.set_at((x, y), figcgapal[c])
...
def figopen(x, s):
    import fileinput
    if s.lower() == "w":
        if (x) not in list(figfilehandles.keys()):
        ...
        if (x) not in list(figfilehandles.keys()): figfilehandles[x] = open(x[:], s[:])
...
def figclose(x):
    if (x) in list(figfilehandles.keys()):
...
def figarrcurl(x, s):
    from urllib import request ; urlopen = request.urlopen
...
def figlineinput(p): return stdin.readline()[:-1]
        ...
        if len(p.lstrip()):
            ...
            if p.lstrip()[0] == "#":
                if linecount == 1:
                    ...
                    #% write comments
                    #print color(14, 0) + p + color(7,0) ; znul = input()  #$
            ...
            #            print lc() + p
                ...
                if not lnob(x, 0) == "figg":
                    ...
                    0) in list(cmds.keys()) and not lnob(x,
                    0) in list(funcs.keys()) + ["forin", "for", "function", "nextin",
                    "next", "while", "wend"] + ["break", "pass"]:
                        if not lnob(x, 0) in vrs: vrs += [lnob(x, 0)[:]]
                        #% write variable
                        #var: print color(14, 0) + "variable:" + lnob(x, 0) + color(7,0) ; znul = input()  #$
                        outfilewrite(outb, "\n")
                    ...
                    if lnob(x, 0) in list(cmds.keys()): # + ufunc.keys():
                        e = 4 ; shln = lnob(x, 0)

                    if lnob(x, 0) != "fig" and lnob(x,
                    0) not in list(funcs.keys()) + ["forin", "for", "function",
                    "nextin", "next", "while", "wend"] + ["break", "pass"]:
                        ...
                        for prs in getlmore(x, 1):
                            ...
                            if cstrctr > 0:
                                ...
                                if cstrctr == 0:
                                    ...
                                    if lnob(x, prsc - 1) == "return":
                                        ...
                                        if figprsbac in list(ufunc.keys()):
                            ...
                            if prs.lower() in list(funcs.keys()) + ["forin", "for",
                            ...
                            if prs.lower() in list(ufunc.keys()):
                            ...
                            if prs.lower() in list(cmds.keys()):
...
if errorin:
    ...
    color(7, None)
    print(errorsrc)
    #from os import system as stf ; p = stf("touch e") # line patched jun 2016
else:
