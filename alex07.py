...
pcmds = ["cd", "while"] # take priority no matter what files are found
...
def qesc(p): return chr(34) + p.replace(chr(34), chr(92) + chr(34)) + chr(34)
...
while 1: 
    ...
    if len(token) or len(ptoken):
        ...
        elif token in ["help", "pserver", "isoname", "minusname", "cat", "arrdo", "fsortplus", "fields", "replace"] and (bufpos == 1 or prevtoken == "|"): token = pyprefix + alexver + ".py --" + token ; x = 1
    ...
    if token == ";" and bufpos > 1: 
        if ptoken: 
            ...
            if alexvar:
                outprog += chr(32) * alexatleast(0, indent) + alexvar + " = alexvarproc(alexarrshell(" + qesc(agg) + "))" + nl ; agg = "" ; prevtoken = "" ; runvar = 0
        else:
            outprog += chr(32) * alexatleast(0, indent) + "alexr = alexshell(" + qesc(agg) + ")" + nl ; agg = "" ; prevtoken = ""
