...
for p in ('addfuncs'    ,'addto'    ,'addtoout'    ,'atleast'    ,'bcolour'
    ...
    ,'prs'    ,'prsc'    ,'right'   ,'rosefsp'    ,'rosefsplit'
    ,'roseprsbac'    ,'roseraphics'    ,'roseunusedvar'    ,'roseusevar'    ,'sgn'
...
for p in inputfile:
    ...
    if linecount == 1:
        ...
        if "," in proginf:
            outfile.write("# rose translator version: " + proginf.split(",")[0] + nl)
            ...
            if p.lstrip()[0] == "#":
                ...
                if not roseusevar == "roseg":
                    ...
                    "next", "while", "wend"] + ["break", "pass"]:
                        ...
                        outfilewrite(outb, chr(32) * atleast(0, indent) +
                        "try: roselist = int(type(" + roseusevar + ") in (list, dict, float, int, str))" + nl)

                        outfilewrite(outb, chr(32) * atleast(0, indent) +
                        "except NameError: pass" + nl)
                        outfilewrite(outb, chr(32) * atleast(0, indent) +
                        "if not roselist: " + roseusevar + " = 0 " + nl)
...
if errorin:
    ...
    ,popen    ,proginf    ,prs    ,prsc    ,right    ,rosefsp
    ,rosefsplit    ,roseprsbac    ,roseraphics    ,roseunusedvar    ,roseusevar
