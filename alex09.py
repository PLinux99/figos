...
try: 
    if sys.argv[1] == "--help": 
        ...
        print "    set varname value           set varname to value"
...
pcmds = ["cd", "while", "set", "setrandint"] # take priority no matter what files are found
...
while 1: 
    ...
    if len(token) or len(ptoken):
        ...
        elif token in ["help", "pserver", "isoname", "minusname", "locate", "colour",
        "cat", "arrdo", "fsortplus", "fields", "replace"] and (bufpos == 1 or prevtoken == "|"): token = pyprefix + alexver + ".py --" + token ; x = 1
