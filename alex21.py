...
try: 
    if sys.argv[1] == "--help": 
        ...
        print "    find | dc [n] [d[srchdate]] find files, list name/size/time, colour by type"
        print
                ...
                for lp in range(0, len(b), lw):
                    ...
                    try:
                        if lw < 2:
                            p = b[lp]
                            alexpld = 0
                            if alexprintlinedc(str(p[3]), dates):
                                alexpld = 1
...
while 1: 
    ...
    if len(token) or len(ptoken):
        ...
        "cat", "arrdo", "fsortplus", "fields", "replace", "rainbow", "ascii", "isoplus", "noreps", "sleep", "dc", "findsim", "lcase", "ucase", "chr", "arrcurl"] and (bufpos == 1 or prevtoken == "|"): token = pyprefix + alexver + ".py --" + token ; x = 1
