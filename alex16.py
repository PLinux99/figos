            ...
            try: 
                ...
                for lp in range(0, len(b), lw):
                    ...
                    try:
                        if lw < 2:
                            p = b[lp]
                            alexprint( alexpadright(p[1], bfext, alexcolourdc(p[0], p[1])) + " " + alexpadleft(p[2], bfs, alexcolourdc(p[0])) + " " + alexpadleft(str(p[3]), bfiletime) + " " + " " )
                        else: 
                            for lpc in range(lw):
                                try: 
                                    p = b[lp + lpc]
                                    alexprints( alexpadright(p[1], bfext, alexcolourdc(p[0], p[1])) + " " + alexpadleft(p[2], bfs, alexcolourdc(p[0])) + " " + alexpadleft(str(p[3]), bfiletime) + " " + " " )
                                except:
                                    pass
                            alexprint("")
                    except: 
...
while 1: 
    ...
    if len(token) or len(ptoken):
        ...
        "cat", "arrdo", "fsortplus", "fields", "replace", "rainbow", "ascii", "isoplus", "noreps", "dc"] and (bufpos == 1 or prevtoken == "|"): token = pyprefix + alexver + ".py --" + token ; x = 1
