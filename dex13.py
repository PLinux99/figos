...
proginf = "dex 1.3, sep 2017 mn" # with lots of design help
    ...
    for ch in each:
        ...
        if (ch == " " or allch == each) and len(allch.strip()):
            if allch.lower().strip() == "clr":
                ...
                allcd = "do that thing" ; outverb(each, allch, allcd)
                break
            ...
            #### nxt - mark the bottom of a for loop or conditional or other command block
                ...
                allcd = "set " + allch.strip() + " to " + dexrt(each, len(each) - len(allch)).strip() 
